...
Revision ID: 2d56a960b9ae
Revises: d8ccc69b835c
Create Date: 2025-05-03 14:42:26.250981
...
revision = '2d56a960b9ae'
down_revision = 'd8ccc69b835c'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.execute("""
        ...
        SELECT id, title, start, user_id FROM events
    """)
...
def downgrade():
    ...
    op.create_table(
        ...
        sa.Column('title', sa.String(length=100), nullable=False),
        sa.Column('start', sa.DateTime(), nullable=False),
        sa.Column('user_id', sa.Integer(), nullable=False),  # Reapply NOT NULL constraint
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='fk_events_user_id')
    )
